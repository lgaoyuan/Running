...
public class Chat_gonglueFetch extends SingleHttpFetch {
    public String fetchItems(String id) {
        String url = Uri.parse("https://lgaoyuan.xyz:8080/running/getSuggest").buildUpon().appendQueryParameter("id", id).build().toString();
        String json = null;
        try {
            json = getUrl(url);
        } catch (IOException e) {
